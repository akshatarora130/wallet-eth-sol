...
pub fn sol_keys() {
    ...
    // This is actual private key that we see in various wallets
    println!("Key Pair -> {}", key_pair.to_base58_string());
...
pub async fn eth_keys() {
    ...
    println!("ETH Public Key -> {}", public_key_hex);

    let private_key_bytes = wallet.signer().to_bytes();
    let private_key = hex::encode(private_key_bytes);
    println!("ETH Private Key -> 0x{}", private_key);
